...
**Live demo:** https://failtrace-livid.vercel.app/
**Screen recording:** https://youtu.be/C4eALFRktRo
...
In the seeded plant the generator scores 14, reaching every other machine, which correctly identifies it as the plant's single point of failure.
...
components/
  ...
  EquipmentDetail.tsx         presentational, takes props, cannot fail
...
3. Copy the connection URI (`bolt+s://<instance-id>.databases.cognodb.cloud`) and the generated password. **The password is shown once**, so save it immediately.
